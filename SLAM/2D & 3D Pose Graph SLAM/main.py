...
def isam_3d(input_file,output_file):
    ...
    # if not os.path.exists('images_isam2_3d'):
    #     os.makedirs('images_isam2_3d')
    ...
    #     ax.plot(poses[:i+1, 1], poses[:i+1, 2], poses[:i+1, 3], label='Unoptimized Trajectory')
    #     ax.plot(opt_poses[:i+1, 9], opt_poses[:i+1, 10], opt_poses[:i+1, 11], label='Optimized Trajectory')
    #     plt.title('isam2 3D')
    #     ax.legend()
    ...
    #     # Save frame
    #     plt.savefig(f'images_isam2_3d/frame_{i:03d}.png')
    #     plt.close(fig)
