...
if __name__ == "__main__":
    ...
    for i in range(pose_number):
        ...
        pose_arr_key = np.array(pose_key)
        if i==0:
            plt.plot(x_slam[pose_arr_key], x_slam[pose_arr_key+1], '-', color=darkblue, alpha=0.7, linewidth=2, label='SLAM')
        else:
            plt.plot(x_slam[pose_arr_key], x_slam[pose_arr_key+1], '-', color=darkblue, alpha=0.7, linewidth=2)
        plt.plot(x_slam[pose_arr_key], x_slam[pose_arr_key + 1], '.', color=darkblue, alpha=0.7, markersize=18)
        # print(x_slam[pose_arr_key])
        # SLAM map
        # landmark_arr_key = np.array(landmark_key)
        # plt.plot(x_slam[landmark_arr_key-1], x_slam[landmark_arr_key], '*', color=darkblue, alpha=0.7, markersize=14)
        plt.legend(loc='best')
