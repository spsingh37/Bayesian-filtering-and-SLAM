...
class ogm_continous_S_CSM:
    ...
    def construct_map(self, pose, scan):
        ...
        # -----------------------------------------------
        self.map['mean'] = self.prior * np.ones([t.shape[0], self.num_classes+1])        # size should be (number of data) x (number of classes + 1)
        self.map['variance'] = self.prior * np.ones([t.shape[0], 1])                     # size should be (number of data) x (1)
        self.map['alpha'] = self.prior_alpha * np.ones([t.shape[0], self.num_classes+1]) # size should be (number of data) x (number of classes + 1)
    ...
    def continuous_S_CSM(self, z, i, k):
        ...
        if d < self.l:
            ...
            self.map['alpha'][i, int(z[idx,2]-1)] = self.map['alpha'][i, int(z[idx,2]-1)] + kernel
        ...
        for r in np.arange(d_o_m-self.l, min(z[idx,0], d_o_m+self.l), self.l*2/3):
            ...
            if dist < self.l:
                ...
                self.map['alpha'][i, self.num_classes] = self.map['alpha'][i, self.num_classes] + kernel
    ...
    def build_ogm(self):
        ...
        for i in tqdm(range(self.map['size'])):
            ...
            if np.array_equal(self.map['alpha'][i], self.prior_alpha * np.ones_like(self.map['alpha'][i])):
                # unkown grid variance
                self.map['variance'][i] = self.map['mean'][i, 0] * (1 - self.map['mean'][i, 0]) / (self.map['alpha'][i, 0] + 1)
            else:
                semantic_class = int(np.argmax(self.map['mean'][i]))
                self.map['variance'][i] = self.map['mean'][i, semantic_class] * (1 - self.map['mean'][i, semantic_class]) / (self.map['alpha'][i, semantic_class] + 1)
