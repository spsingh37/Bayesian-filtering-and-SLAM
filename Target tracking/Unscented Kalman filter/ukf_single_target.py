...
# Uncomment the following (measurement model Jacobian) if also using EKF for comparision 
...
# Example for tracking a single target using an Unscented Kalman filter and
...
if __name__ == '__main__':
    ...
    ###################################################################
    # Extended Kalman Filter (Uncomment the following for comparision, similarly the code for plot)
    ...
    gt_x = gt.x
    gt_y = gt.y
    pred_x = x_ukf[:, 0].flatten()
    pred_y = x_ukf[:, 1].flatten()
    ...
    line4, = plt.plot(x_ukf[:, 0], x_ukf[:, 1], '--r', linewidth=1.5)
    ...
    dot_pred, = ax.plot([], [], 'ro', label='UKF Predictions')  # Initialize the prediction dot
    ax.legend()
    ...
    ani.save("ukf_sim.mp4", writer=writer)
